...
| CATALOG ENGINE | **무질서→구조 카탈로그 엔진 (14 메타메커니즘) — PHASE A–F 완료.** `mechanisms/`(14 메커니즘 1..14 + 2 원시기법, 프레임워크 닫힘·15번째 없음) + `catalog/`(94 변환 = §4 명명 대표, 9패스·14메커니즘 전부 매핑, 38 합성). 카탈로그 커버리지 **94 등록 / 9 VERIFIED / 85 정직-defer**(전부 UNVERIFIED(사유)·kernel=None — 가짜 통과 0). VERIFIED 커널 7개(kernel_router.REGISTRY): **SOS/Positivstellensatz**(`sos_cert.py`, 새 EXACT tier·유리 PSD Gram·Sturm-exact PSD), Presburger(z3), RCF/CAD(`mathmode.real_qe`), 순서수-종료(`ordinal_cert.py`, fold decreases-clause), MDL-비압축성(`decline_boundary`, 측정 zlib·가짜-Ω(N) 회수), Buckingham-Π, Noether. §5 합성 라우터(`catalog/compose.py`): arith-hierarchy→DECLINE가드→fold[13]→MDL[12]→합성(SOS[4] 등), `(result,grade,cert,bound,mechanism_path)` 반환. §6 DECLINE 백본(Rice/비압축성/turbulence + 15 경계, 위양성 0). `test_catalog.py` **18/18 green**(test_build 273 영향 없음 — 순수 가산). 상세·정직 defer 목록: `BUILD_LOG_catalog.md` §C. |
