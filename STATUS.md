...
| Tests | **276 passed / 276** — `OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 NUMBA_NUM_THREADS=1 MKL_NUM_THREADS=1 python3 test_build.py` (+1 §MRJ provider-wiring · +1 §BE browser-offload/isolation · +1 §BF `-O` soundness-gate regression); `test_catalog.py` **239** (+4 §BA caps · +1 §SEC search-gate · +1 §BB R-1 slice-split · +1 §BC CA-1 causal-poset · +1 §BD checker-layer · +1 §BF DECLINE diagnostics · +1 §BG past-native+runtimes) |
