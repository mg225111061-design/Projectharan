...
| Tests | **216 passed / 216** — `OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 NUMBA_NUM_THREADS=1 MKL_NUM_THREADS=1 python3 test_build.py` |
...
| Now building | **UNIFIED ARSENAL** (a transform system + b ~70 fold families + c physics) — foundational-first: §1 foundations ✅ (G1·G2·G3·G4) → §2 decision procedures: summation ✅ · integration ✅ (Risch·Kovacic) → CAD/QE decision procedures → §3 physics → §4 transforms. (NATIVE-CORE done: `NATIVE_CORE_REPORT.md`.) |
