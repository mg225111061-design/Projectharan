...
_A: List[Algo] = [
    ...
    Algo(12, "NTT / FFT convolution", "A", "pillar3.convolution", "conv_ntt", KV.EXACT,
         ...
         False),
    Algo(13, "Bostan–Mori (GF coefficient extraction)", "A", "", "", KV.EXACT,
         "GF defining equation; [x^n] re-checked at small n", "O(M(d) log n)", False, NORMAL, False, GAP),
    Algo(14, "Newton iteration on power series", "A", "", "", KV.EXACT,
         "series identity to truncation order (inv·orig ≡ 1, etc.)", "quadratic convergence, O(M(n))", False,
         NORMAL, False, GAP),
    ...
    Algo(19, "Gröbner basis (Buchberger / F4)", "A", "", "", KV.EXACT,
         "cofactor representation / S-poly reduction to zero", "EXPSPACE worst case — extend-budgeted", True,
         EXTEND, False, GAP),
...
_B: List[Algo] = [
    ...
    Algo(28, "Automatic differentiation (exact dual)", "B", "", "", KV.EXACT,
         "chain-rule identity re-checked symbolically", "O(ops) forward/dual", False, NORMAL, False, GAP),
...
_C: List[Algo] = [
    Algo(31, "Fast modular exponentiation", "C", "mathmode.number_theory", "modexp_grade", KV.EXACT,
         ...
         True),
    Algo(32, "Power towers via Carmichael-λ", "C", "", "", KV.EXACT,
         "Euler/Carmichael validity (gcd checked) for a^(b^c) mod m", "O(log m) per level", False, FAST, True, GAP),
    ...
    Algo(34, "Lucas' theorem + Granville lifting", "C", "", "", KV.EXACT,
         "modular identity C(n,k) ≡ ∏ C(nᵢ,kᵢ) (mod p); p^e lift", "O(log_p n) for astronomical n", False, FAST,
         True, GAP),
