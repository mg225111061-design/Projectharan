"""
HARAN §4 — TIER ROUTING for the 50 algorithms: normal (~30s) / extend (~8min BOUNDED) + broth. (2-tier — a
former third tier, `fast`, retired; algorithms it used to host are tagged `normal`, the new floor.)
==========================================================================================================
The operational glue that puts §1 (the 50 named algorithms, each tagged with the tier its certified path runs
under) together with §2 (the broth) and the normal/extend CONTRACT in `pillar3/mode.py`. The routing rules,
enforced here and tested per-commit:
  • A BROTH HIT short-circuits in ANY mode — the result was pre-proven offline, so it returns INSTANTLY (O(1)
    lookup) EVEN in normal, regardless of how heavy the underlying algorithm is. (The UI shows "사전증명된 닫힌형 0.1µs".)
  • On a MISS, the algorithm runs ONLY if its tier ≤ the requested mode. normal (~30s) NEVER runs an extend-tier
    heavy decision procedure (CAD/Risch/Gröbner/Kovacic/Petkovšek/ΠΣ*/factorization/Lucas–Lehmer/BSGS) — it
    returns TIER_UP, telling the caller to escalate. extend runs everything within its BOUNDED 8-minute budget.
This is the contract from `ModePolicy`: normal=CHEAP_CERT (never the heavy solver), extend=FULL_CERT
EXACT-or-DECLINE, both time-bounded. The router decides ROUTING ONLY — it never weakens a grade and never runs
past a budget.
"""
...
def can_run(algo_num: int, mode: str) -> bool:
    """Can algorithm `algo_num` run a full certified pass in `mode`? True iff its tier ≤ the mode (normal never
...
def route(algo_num: int, mode: str, broth_key: Optional[Tuple] = None) -> dict:
    ...
    Returns one of:
      • {action: "BROTH_HIT", ...}  — pre-proven; instant O(1) result, valid in ANY mode (even normal)
      • {action: "RUN", ...}        — a MISS, but the algorithm's tier ≤ mode ⇒ run it in this tier
      • {action: "TIER_UP", ...}    — a MISS and the algorithm is heavier than the mode ⇒ escalate (normal
                                       won't call the heavy solver). The required tier is named."""
    ...
    return {"action": "TIER_UP", "algo": algo_num, "mode": mode, "required_tier": algo.tier,
            "ui": f"#{algo_num} {algo.name}: {mode} 예산 초과 — {algo.tier} 필요 (heavy solver, normal 금지)"}


def routing_matrix() -> dict:
    """The full {algo × mode} routing decision (miss case), for the per-commit invariant test. Asserts the
    normal-never-heavy-solver contract holds for every one of the 50."""
    rows = {}
    for a in A.ALGOS:
        rows[a.num] = {m: route(a.num, m)["action"] for m in ("normal", "extend")}
    normal_heavy = [a.num for a in A.ALGOS if a.tier == "extend" and rows[a.num]["normal"] != "TIER_UP"]
    extend_runs_all = all(rows[a.num]["extend"] == "RUN" for a in A.ALGOS)
    return {"rows": rows, "normal_hosts_no_heavy_solver": not normal_heavy,
            "normal_tier_up_count": sum(1 for a in A.ALGOS if rows[a.num]["normal"] == "TIER_UP"),
