...
PURE, IO, NONDET = "pure", "io", "nondet"


@dataclass
class Effect:
    effect: str                          # "pure" | "io" | "nondet"
    reason: str = ""


def classify_effect(src: str) -> Effect:
    """nondet > io > pure (conservative). nondet is a permanent DECLINE (no ∀-input determinism)."""
    t = AT.tag(src)
    if t.nondet:
    ...
    return Effect(PURE, "no io / nondet effect ⇒ a pure fold candidate")
