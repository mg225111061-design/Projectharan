"""
MATH-Ascent §1 — the two TOP-LEVEL modes: CODE and MATH (fast/normal/extend preserved INSIDE each).
=====================================================================================================
...
default first-move + verifier emphasis, and — critically — the OMEGA §B fast/normal/extend sub-separation is
preserved verbatim INSIDE each top mode (fast never calls Z3; extend is EXACT-only; every tool is mode-tagged).
A per-commit invariant test asserts CODE and MATH route measurably differently AND that the inner separation
holds in both. Blurring fails the build.
"""
...
def route(top_mode: TopMode) -> TopRoute:
    ...
    their first move and central tool; their toolsets differ; both preserve the inner fast/normal/extend §B."""
...
def inner_modes(top_mode: TopMode) -> List["M.ModePolicy"]:
    """The OMEGA fast/normal/extend sub-modes, preserved verbatim INSIDE the chosen top mode. The §B contract is
    identical in CODE and MATH: fast=MICRO/no-Z3, extend=EXACT-or-DECLINE."""
    return [M.ModePolicy.for_mode(m) for m in (M.Mode.FAST, M.Mode.NORMAL, M.Mode.EXTEND)]
