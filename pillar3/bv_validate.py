...
def bv_equiv_inhouse(build: Callable, bits: int = 8) -> Tuple[str, Optional[dict]]:
    ...
    external solver, and to cross-check Z3 where both apply. NOT Z3 parity (no division or variable-shift; signed
    compare, general multiply, right-shift, and ite-mux ARE in-house)."""
    import os
    import sys
...
def cross_check_inhouse_vs_z3() -> dict:
    ...
    NOT cross-checked here — they are unsound (and one needs division); the in-house solver can refute the
    conditional ones via ite-mux, but they are not SOUND peepholes, so they stay out of this cross-check."""
    import os
    import sys
