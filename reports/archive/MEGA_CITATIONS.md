...
| metamorphic relations + cross-checking (PHASE M, `metamorphic.py`) | Chen, Cheung, Yiu (metamorphic testing, 1998); Segura et al. survey, **TSE 2016** | relations a transform must preserve (sort permutation-invariance/idempotence/multiset; sum order-invariance). We use it ONLY to downgrade a borderline pass to DECLINE — never to manufacture a win. |
| concolic / Z3-guided input generation (PHASE I, `inputgen.py`) | DART (Godefroid et al., PLDI 2005) / CUTE (Sen et al., FSE 2005) | we solve a branch predicate true AND false (concolic-*lite*: one predicate at a time, not full path exploration) to cover both sides; δ from real sample size (rule of three). |
| Kadane / Boyer-Moore / two-sum recognizers (PHASE A, `algorithms.py`) | classic algorithms (Bentley 1984 max-subarray; Boyer-Moore majority 1981) | recognized → optimal form, graded PROBABILISTIC (control flow ⇒ Z3 bounded-validation doesn't apply); measured whole-program with n stated, adversarial wrong → DECLINE. |
